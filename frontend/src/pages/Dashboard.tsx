...
import JobFilterSection, {
  JobFilters,
} from "../components/jobs/JobFilterSection";
...
const Dashboard: React.FC = () => {
  ...
  const filteredJobs = useMemo(() => {
    return allJobs.filter((job) => {
      const { jobTitle, jobType, location, experienceLevel, salaryRange } =
        filters;
      ...
      const salaryMatch =
        salaryRange === "All" || job.salary === salaryRange;
  ...
  const welcomeMessage = authLoading
    ? "Loading..."
  ...
  return (
    ...
    >
      ...
      <h2 className="text-3xl text-white mt-10 mb-4">
        Recommended Jobs
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="bg-gray-800 p-6 rounded-lg shadow-lg h-56 animate-pulse"
            />
          ))
        ) : allJobs.length === 0 ? (
          <div className="md:col-span-3 text-center py-16 bg-gray-800/50 rounded-lg">
            ...
            <p className="text-gray-500 text-sm">
              Jobs will be automatically updated every 12 hours.
            </p>
          </div>
